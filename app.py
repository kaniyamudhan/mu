...
def get_response():
    ...
    if next_prompt:
        ...
        if not is_valid:
            ...
            response_message = f"Thank you {conversation['name']}! You have booked {conversation['tickets']} ticket(s) for {conversation['date']} at {conversation['time']}."
            
            # Proceed to payment
            response_message += f" Please proceed to [payment](/create-checkout-session/{user_id})."

    return jsonify({"response": response_message})

@app.route('/create-checkout-session/<user_id>', methods=['GET'])
